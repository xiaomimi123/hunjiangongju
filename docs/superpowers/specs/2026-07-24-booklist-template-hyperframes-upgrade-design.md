...
## 实测结论（2026-07-24，本地真渲染）

对样例任务 `96622be4`（书《活下去的理由》，10 段，带精确 captionBeats）用三套预设各真跑
`hyperframes@0.7.33 render`（本机 chrome-headless-shell，720×960，37.6s，均出片成功）并抽帧验收：

- **三套预设视觉可区分且都成立**：warm-literary（烫金强调 + 宋体标题）、dark-premium（烫金 +
  黑体标题 + 更深压暗底）、ink-oriental（朱砂强调 + 颗粒质感 + 宋体标题）。
- **美术系统生效**：常驻书名头（书名 +「著」+ 强调色 kicker 短杠）、双语字幕 + 渐变压暗底
  （任意底图上均清晰）、油画原图透出。
- **char-stagger 保留**：逐字浮现在无头 seek 下正常——稳态帧字幕字符完整可见（若失效会缺字），
  故 `ENTRANCES` 维持四型不降级。
- 运镜/转场为逐帧渐变，静帧不易判读，但时间轴与出片流程验证无误（总时长 == 音频 37.6s）。

### 待微调（非阻断，留后续美术迭代）
- ink-oriental 的作者行用朱砂红（#c1272d），在深色油画上对比偏低、略难读 → 后续可给作者行也叠
  文字阴影或改用 `--ink-dim`。
- 该样例任务各段图片高度相似（数据所致，非模板问题），运镜多样性在图片有差异时更明显。
