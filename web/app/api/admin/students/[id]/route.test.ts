...
describe('[id] route — 积分充值', () => {
  it('充值 1000cc（10 积分）→ 余额 +1000，且记一条充值流水（含操作人）', async () => {
    const s = await makeUser('student')
    requireRoleMock.mockResolvedValueOnce({ userId: 'op-recharge', role: 'operator' })
    const res = await PATCH(patchReq({ action: 'recharge', amount: 1000 }), { params: { id: s.id } })
    expect(res.status).toBe(200)
    expect((await prisma.user.findUniqueOrThrow({ where: { id: s.id } })).credits).toBe(4000) // 新号默认 3000cc(30 积分) + 1000cc
    const logs = await prisma.creditLog.findMany({ where: { userId: s.id } })
    expect(logs).toHaveLength(1)
    expect(logs[0]).toMatchObject({ delta: 1000, reason: 'recharge', operatorId: 'op-recharge' })
  })
  ...
  it('金额非法（0 / 负数 / 非整数）→ 400，不动余额不记流水', async () => {
    ...
    expect((await prisma.user.findUniqueOrThrow({ where: { id: s.id } })).credits).toBe(3000)
