...
- 工具广场：从扣子工作流工具库选工具、填表单、运行获取结果（文本/图片/视频/网页链接），支持图片上传、结果可复制/下载、查看运行记录；工具页可看运营配置的成品演示与使用教学视频
- AI 实拍生图：学员拍书籍封面/内页照片，图生图生成真实感种草照（封面 5 种拍法+智能推荐、内页清透窗光/暗调，内页自动视觉识别版位）；按张扣积分、失败自动退，生图记录可回看。生图服务在「模型配置→实拍生图」配置（须支持图片输入）
- 积分体系：最小单位 0.01 积分（库内厘分整数记账无浮点误差）；视频全局单价在「系统设置」可调，且框架库可给每个框架设专属价（留空用全局价，学员端框架卡直接显示价格）；生图单价在「模型配置→实拍生图」高级参数（如 0.08/张），扣子工具单价逐工具设置；扣费/退分全部入积分流水，后台学员明细逐笔可查
- 积分流水看板：后台「积分流水」页——近 30 天充值/消耗/退回统计、消耗按产品构成、消耗大户与沉睡户排行、全站流水表（按学员/类型筛选）
- 每日余额日报：worker 每天北京时间 9 点查询生图服务余额（GET /v1/balance）+ 昨日用量，发邮件到「系统设置→运营告警」配置的邮箱（依赖 SMTP 开启）；余额告急主题带【告急】。后台另有实时红横幅兜底
...
- 扣子工具箱：导入扣子工作流、配中文标签/输入类型/价格、上下架、查看学员运行记录；全站一个扣子 PAT Token 在「模型配置」配置。输入类型含「固定值」——后台配死的参数（如第三方 api key）由服务端运行时注入，学员端不显示也拿不到
