...
1. **快闪卡（可精确复刻）**：快闪卡数由书单本数决定，但「卡与卡之间是硬切还是淡入」是**全局性质**，与数量无关。若草稿的快闪段边界全是硬切 → `flashCardsTweens` 改为瞬时切换（`tl.set` 而非 0.12s `fromTo`）。同时把 `flash.bounceIn` 从硬编码改为**真从草稿判定**（有无对应入场动画）。**这一项是本阶段视觉收益最大的改动，且能做到精确。**
