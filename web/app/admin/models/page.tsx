...
const LABELS: Record<string, { name: string; hint: string }> = {
  ...
  photo: { name: '实拍生图', hint: '学员拍书生成真实感种草照（图生图）' },
}
          ...
          <div key={c.capability} id={`cap-${c.capability}`} className="card space-y-2.5 p-4">
            ...
            {c.capability === 'photo' && (
              <p className="text-xs text-ink3">
                ...
                高级参数可设 {'{"resolution":"1k","pricePerImage":1}'}——resolution 生成分辨率（1k/2k），pricePerImage 学员每张消耗积分（默认 1，支持两位小数如 0.08；也可到「系统设置 → 积分定价」直接改）。
